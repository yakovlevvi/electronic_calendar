...
def back(event):
    ...
    if month == 0:
        month = 12
        year -= 1
    bg = PhotoImage(file = "files\months\{}.png".format(month))
    canvas1.create_image(0, 0, image = bg, anchor = "nw")
    canvas1.create_image(15, 30, image = era, anchor = "nw")
    canvas1.create_image(985, 30, image = morf, anchor = "ne")
    fill()
...
def fill():
    ...
    for n in range(month_days):
        ...
        if year == now.year and month == now.month and n == now.day:  #сегодняшний день
            ...
            days[n + week_day - 1]['activeforeground'] = 'black'
            days[n + week_day - 1]['disabledforeground'] = 'black'
...
info_label_canvas = canvas1.create_window(500, 50, anchor = "n", window = info_label)
...
back_button_canvas = canvas1.create_window( 50, 400, anchor = "w", window = back_button)
...
next_button_canvas = canvas1.create_window( 900, 400, anchor = "w", window = next_button)

# Пн-Вс
canvas2 = Canvas(root, width=700, height = 30)
...
for n in range(7):
    ...
    if n == 5 or n == 6:
        lbl['fg'] = 'red'
    lbl_canvas = canvas2.create_window(50+100*n, 0, 
                                       anchor = "n",
                                       window = lbl)
...
for row in range(6):
    for col in range(7):
        lbl = Button(root, text = '0', width= 4, borderwidth=0)
        lbl.configure( font=("Century Gothic", 20, 'bold'))
        lbl.bind('<Button-1>', show_info)
        lbl_canvas = canvas1.create_window(165+100*col, 200+100*row, anchor = "w", window = lbl)
        days.append(lbl)

# 2024
next_year_button = Button(root, text = 'Перейти на 2024 г.', height = 1, borderwidth=0)
next_year_button.bind('<Button-1>', next_year)
next_year_button.configure( font=("Century Gothic", 20))
canvas1.create_window(250, 790, anchor = "s", window = next_year_button)
...
current_year_button.configure( font=("Century Gothic", 20))
canvas1.create_window(680, 790, anchor = "s", window = current_year_button)
...
# ERA text
era_label = Label(root, text='ВОЕННЫЙ ИННОВАЦИОННЫЙ ТЕХНОПОЛИС "ЭРА". Лаборатория гидрометеорологического и геофизического обеспечения', 
                  fg='red')
era_label.configure( font=("Century Gothic", 10))
canvas1.create_window(500, 14, anchor = "n", window = era_label)
